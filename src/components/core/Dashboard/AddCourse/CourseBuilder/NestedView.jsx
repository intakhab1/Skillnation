...
export default function NestedView({ handleChangeEditSectionName }) {
  ...
  // States to keep track of mode of modal [ ADDING , VIEWING , EDITING ] LECTURE
  ...
  const handleDeleteSubSection = async (subSectionId, sectionId) => {
    const result = await deleteSubSection({ subSectionId, sectionId, token }) // returns -> updated section data
    if (result) {
      // creating updated course using updated section data
      const updatedCourseContent = course.courseContent.map((section) => 
      section._id === sectionId ? result : section)
      const updatedCourse = { ...course, courseContent: updatedCourseContent }
      dispatch(setCourse(updatedCourse))
    }
    setConfirmationModal(null) // null/false -> closes the delete-confirmation modal
  }
        ...
        {course?.courseContent?.map((section) => (
          <details key={section._id} open>
            ...
            <summary className="flex cursor-pointer items-center justify-between border-b-2 border-b-richblack-600 py-2">
              <div className="flex items-center gap-x-3"> <RxDropdownMenu className="text-2xl text-richblack-50" />
              ...
              {/* 1-EDIT BUTTON ICON */}
                ...
                {/* 3- DROPDOWN ICON */}
                <span className="font-medium text-richblack-300">|</span> <AiFillCaretDown className={`text-xl text-richblack-300`} />
...
{/* ************************************************ SUB- SECTION ******************************************************************* ******/}
            ...
            <div className="px-6 pb-4">
              ...
              {section.subSection.map((data) => (
                <div key={data?._id} onClick={() => setViewSubSection(data)} className="flex cursor-pointer items-center justify-between gap-x-3 border-b-2 border-b-richblack-600 py-2">
                  ...
                  {/* DROPDOWN ICON */}
                  <div className="flex items-center gap-x-3 py-2 "> <RxDropdownMenu className="text-2xl text-richblack-50" />
              ...
              {/* Add Lecture BUTTON -> add new sub-section */}
      ...
      {/* MODES -> ADDING , VIEWING , EDITING Modal Display */}
      ...
      {/* RENDER delete-Confirmation Modal -> but first validate if data is present or not */}
