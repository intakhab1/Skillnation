...
function CourseDetailsCard({ course, setConfirmationModal, handleBuyCourse }) {
  ...
  const handleAddToCart = () => {
    // CHECK IF USER is valid and  IS NOT INSTRUCTOR
    ...
    // IF USER IS NOT LOGGEN IN -> SHOW POP UP/ MODAL
        ...
        <div className="px-4">
          ...
          <div className="flex flex-col gap-4">
            ...
            <button
              ...
              onClick={ user && course?.studentsEnroled.includes(user?._id) ? () => navigate("/dashboard/enrolled-courses"): handleBuyCourse}
          ...
          <div className={``}>
            ...
            {/* INSTRUCTIONS -> map to show all the instructions  */}
            <div className="flex flex-col gap-3 text-sm text-caribbeangreen-100">
              {course?.instructions?.map((item, i) => {
                return ( <p className={`flex gap-2`} key={i}> <BsFillCaretRightFill /> <span>{item}</span> </p>
                )
              })}
            </div>
          </div>
